...
tvh_video_context_encode(TVHContext *self, AVFrame *avframe)
{
    ...
    // Take care of field rate deinterlace configuration setting
    if (self->field_rate > 1) {
      avframe->pts /= self->field_rate;
      if (avframe->duration > 0)
        avframe->duration /= self->field_rate;
    }

    // Fallback incase we have an invalid duration
    if (avframe->duration <= 0) {
      avframe->duration = self->oavctx->ticks_per_frame;
      tvh_context_log(self, LOG_TRACE, "Frame had invalid duration, setting to default (%"PRId64")", avframe->duration);
    }

    if (avframe->duration != self->oavctx->ticks_per_frame) {
      tvh_context_log(self, LOG_WARNING, "Frame at pts %"PRId64" has unusual duration - expected %d but got %"PRId64"",
                                         avframe->pts, self->oavctx->ticks_per_frame, avframe->duration);
    }
