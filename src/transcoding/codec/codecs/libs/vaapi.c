...
#define VAAPI_DEINT_METHOD_DEFAULT      0
#define VAAPI_DEINT_METHOD_BOB          1
#define VAAPI_DEINT_METHOD_WEAVE        2
#define VAAPI_DEINT_METHOD_MADI         3
#define VAAPI_DEINT_METHOD_MCDI         4
...
static htsmsg_t *
deinterlace_vaapi_method_get_list( void *o, const char *lang )
{
    static const struct strtab tab[] = {
        { N_("Default"),                                 VAAPI_DEINT_METHOD_DEFAULT },
        { N_("Bob Deinterlacing"),                       VAAPI_DEINT_METHOD_BOB },
        { N_("Weave Deinterlacing"),                     VAAPI_DEINT_METHOD_WEAVE },
        { N_("Motion Adaptive Deinterlacing (MADI)"),    VAAPI_DEINT_METHOD_MADI },
        { N_("Motion Compensated Deinterlacing (MCDI)"), VAAPI_DEINT_METHOD_MCDI },
    };
    return strtab2htsmsg(tab, 1, lang);
}
 ...
 */
    int platform;
/**
 * VAAPI Deinterlace method [deinterlace_vaapi mode parameter]
                             ...
                             AVDictionary **opts)
{
    ...
    AV_DICT_SET(opts, "deinterlace_vaapi_method", self->deinterlace_vaapi_method, AV_DICT_DONT_OVERWRITE);
    AV_DICT_SET(opts, "deinterlace_vaapi_rate",   self->deinterlace_vaapi_rate,   AV_DICT_DONT_OVERWRITE);
    AV_DICT_SET(opts, "deinterlace_vaapi_auto",   self->deinterlace_vaapi_auto,   AV_DICT_DONT_OVERWRITE);
...
static const codec_profile_class_t codec_profile_vaapi_class = {
    {
        ...
        .ic_properties = (const property_t[]){
            ...
            {
                .type     = PT_INT,
                .id       = "deinterlace_vaapi_method",
                .name     = N_("VAAPI deinterlacing mode"),
                ...
                .off      = offsetof(tvh_codec_profile_vaapi_t, deinterlace_vaapi_method),
                .list     = deinterlace_vaapi_method_get_list,
                .def.i    = 0,
            },
            {
                .type     = PT_INT,
                .id       = "deinterlace_vaapi_rate",
                .name     = N_("VAAPI deinterlacing rate type"),
                .desc     = N_("Frame Rate: combine the two interlaced fields to create a single progressive frame"
                               "Field Rate: treat each field as a separate entity, each of which are analyzed individually before combining these to create the frames"
                               "Note: with field rate deinterlacing the resulting file will have double frame-rate (for example 25i becomes 50p)"
                               "      This can result in smoother video since the original temporal characteristics of the interlaced video are retained"
                               ),
                .group    = 2,
                .opts     = PO_ADVANCED | PO_PHIDDEN,
            ...
            },
            {
                ...
                .name     = N_("VAAPI only deinterlace fields"),
                ...
                .list     = deinterlace_vaapi_rate_auto_list,
                .def.i    = 0,
            },
            {
                .type     = PT_INT,
